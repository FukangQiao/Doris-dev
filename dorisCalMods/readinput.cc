...
// ====== Initialization, defaults ======
  ...
  generalinput.memory           = 500000000;            // default memory (500MB)
...
//____RaffaeleNutricato START MODIFICATION SECTION 4
  ...
  m_oversample.applyCal 		 = false; 				 // Default for applying calibration factor [JPF]
  ...
  s_oversample.applyCal 		 = false; 				 // Default for applying calibration factor [JPF]
...
// **********************************************************************
	else if (!strcmp(keyword,"M_OVS_APPLY_CAL"))         // Apply calibration to create beta0.
	  {
		m_oversample.applyCal = false;
	    keyword =  word[1] ;      // pass keyword
	    writearg(keyword);
	    toupper(keyword);
	    if (!strcmp(keyword,"ON"))
	      m_oversample.applyCal = true;
	    else if (!strcmp(keyword,"OFF"))
	      m_oversample.applyCal = true;
	  }
// **********************************************************************
	else if (!strcmp(keyword,"S_OVS_APPLY_CAL"))         // Apply calibration to create beta0.
	  {
		s_oversample.applyCal = false;
		keyword =  word[1] ;      // pass keyword
		writearg(keyword);
		toupper(keyword);
		if (!strcmp(keyword,"ON"))
		  s_oversample.applyCal = true;
		else if (!strcmp(keyword,"OFF"))
		  s_oversample.applyCal = true;
	  }
...
// **********************************************************************
      ...
      if (coarsecorrinput.MasksizeL > 2048 || coarsecorrinput.MasksizeP > 2048)
              {
