...
slcimage::slcimage()
  ...
  f_DC_a1                 = 0.0;
  f_DC_a2                 = 0.0;
  calFactor 			  = 1.0; 				// default no conversion
...
void slcimage::fillslcimage(const char* file)
  ...
  bool found_cal 	    = false; // [JPF 05.2009]
  ...
  while(resfile && linecounter<1000)
    ...
    else if (!strcmp(word,"Calibration_constant:"))
      {
        found_cal = true;
        resfile >> calFactor;
        DEBUG << "[" << linecounter
             << "]: string: \"Calibration_constant:\",     (derived) value: "
             << calFactor;
        DEBUG.print();
        if (abs(calFactor) > 0.1)
          WARNING.print("calibration factor > 0.1?");
      }
  ...
  if (found_cal        == false) WARNING.print("found_cal==false");
  else { DEBUG << "calibration factor: " << calFactor; DEBUG.print(); }
