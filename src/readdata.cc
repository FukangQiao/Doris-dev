...
void  OversampleSLC(
  ...
  if (imageinfo.calFactor > 1.0)
  {
	  ERROR << "Calibration factor > 1: " << imageinfo.calFactor;
	  ERROR.print();
	  throw(some_error);
  }
  RN_DSP_CPXACCURACY sqrtCal = RN_DSP_CPXACCURACY(sqrt(imageinfo.calFactor), 0);

  if (imageinfo.calFactor != 1.0 && oversampleinput.oformatflag != FORMATCR4 &&
		  oversampleinput.applyCal)
	  WARNING.print("Apply calibration flag set, calFactor != 1, but output format is not CR4!");
  else if (imageinfo.calFactor != 1.0 && oversampleinput.oformatflag == FORMATCR4 &&
		  oversampleinput.applyCal)
  {
	  INFO << "Applying calibration factor " << imageinfo.calFactor;
	  INFO.print();
  }
    ...
    switch (imageinfo.formatflag)
      ...
      case FORMATCR4:
        ...
        for (int32 ii=0; ii<numpixels; ++ii)
          ...
          // Multiply the input values by the calibration factor if the calFactor
          // isn't 1 and the output is CR4. This produces a true beta0 image.
          // [JPF 05.2009]
          if (imageinfo.calFactor != 1.0 && oversampleinput.oformatflag == FORMATCR4 &&
        		  oversampleinput.applyCal)
        	  LINE_IN(0,OsrRange*ii) *= sqrtCal;
          }
        break;
        }
